...
Note: This requires GCC >= 4.8 for C++11 support (including thread-local).  To use a compiler different than the default (for example on OS X), use:
...
Note: Building `riscv-tools` requires GCC >= 4.8 for C++11 support (including thread-local). To use a compiler different than the default (for example on OS X), you'll need to do the following when the guide requires you to run `build.sh`:
...
##  Installing the Toolchain (11.81 + &epsilon; SBU)
...
##  Building the Linux Kernel (0.40 + &epsilon; SBU)
...
(Instructions for mounting provided courtesy of a\_ou.)
